[index.html](https://github.com/user-attachments/files/26627399/index.html)[Uploading<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8" />
...
.banner-divider { width: 1px; height: 36px; background: rgba(255,255,255,0.15); }
.banner-text h1 { font-size: 19px; font-weight: 700; color: #fff; margin-bottom: 2px; letter-spacing: -.3px; }
.banner-text p { font-size: 12px; color: rgba(255,255,255,.5); font-weight: 400; }
.admin-pill { background: rgba(245,184,0,.15); color: var(--yellow); font-size: 12px; font-weight: 700; padding: 7px 16px; border-radius: 40px; cursor: pointer; border: 1.5px solid rgba(245,184,0,.3); font-family: inherit; white-space: nowrap; transition: all .2s; letter-spacing: .02em; }
.admin-pill:hover { background: rgba(245,184,0,.28); border-color: var(--yellow); }
...
/* Cards */
.card { background: var(--white); border: 1px solid var(--border); border-radius: var(--radius); padding: 1.75rem; margin-bottom: 1.25rem; box-shadow: var(--shadow-sm); }

/* Reminder */
.reminder-box { background: var(--yellow-l); border: 1.5px solid var(--yellow-m); border-radius: var(--radius-sm); padding: .9rem 1.25rem; margin-bottom: 1.5rem; display: flex; align-items: center; gap: .75rem; font-size: 13.5px; color: var(--yellow-d); font-weight: 600; }
.reminder-icon { font-size: 17px; flex-shrink: 0; }

/* Form */
label.field-label { font-size: 11px; font-weight: 700; letter-spacing: .06em; color: var(--grey-m); text-transform: uppercase; display: block; margin-bottom: 7px; }
input, select, textarea { font-family: inherit; font-size: 14.5px; padding: 11px 14px; border: 1.5px solid var(--border); border-radius: var(--radius-sm); background: var(--grey-ll); color: var(--grey); outline: none; width: 100%; transition: border-color .15s, box-shadow .15s; font-weight: 500; }
input:focus, select:focus, textarea:focus { border-color: var(--yellow); box-shadow: 0 0 0 3px rgba(245,184,0,.18); background: #fff; }
input[readonly] { background: var(--grey-l); color: var(--grey-m); cursor: default; }
textarea { min-height: 92px; line-height: 1.6; resize: vertical; }
.grid2 { display: grid; grid-template-columns: 1fr 1fr; gap: 1.25rem; }
.full { grid-column: 1 / -1; }

/* Buttons - Apple style */
.btn-primary { font-family: inherit; font-size: 15px; font-weight: 700; padding: 14px 0; background: var(--yellow); color: var(--dark); border: none; border-radius: var(--radius-sm); cursor: pointer; width: 100%; margin-top: 1.25rem; letter-spacing: -.1px; transition: all .18s; box-shadow: 0 2px 8px rgba(245,184,0,.35); }
.btn-primary:hover { background: #FFD000; transform: translateY(-1px); box-shadow: 0 4px 12px rgba(245,184,0,.45); }
.btn-primary:active { transform: translateY(0); box-shadow: 0 1px 4px rgba(245,184,0,.3); }
.btn-primary:disabled { background: var(--border); color: var(--grey-m); cursor: not-allowed; transform: none; box-shadow: none; }
.btn-dark { font-family: inherit; font-size: 14px; font-weight: 700; padding: 12px 0; background: var(--dark); color: #fff; border: none; border-radius: var(--radius-sm); cursor: pointer; width: 100%; transition: all .18s; letter-spacing: -.1px; }
.btn-dark:hover { background: var(--dark2); }
.btn-ghost { font-family: inherit; font-size: 14px; font-weight: 600; padding: 12px 0; background: var(--grey-l); color: var(--grey); border: 1.5px solid var(--border); border-radius: var(--radius-sm); cursor: pointer; width: 100%; transition: all .18s; letter-spacing: -.1px; }
.btn-ghost:hover { background: var(--border); }
.btn-note { text-align: center; font-size: 12px; color: var(--grey-m); margin-top: 12px; font-weight: 500; }

/* Stats */
.stat-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 1rem; margin-bottom: 1.5rem; }
.stat-card { background: var(--white); border: 1px solid var(--border); border-radius: var(--radius); padding: 1.4rem 1.5rem; box-shadow: var(--shadow-sm); }
.stat-val { font-size: 32px; font-weight: 700; color: var(--dark); font-family: 'DM Mono', monospace; margin-bottom: 3px; letter-spacing: -1px; }
.stat-lbl { font-size: 12px; color: var(--grey-m); font-weight: 600; text-transform: uppercase; letter-spacing: .04em; }
.stat-accent { color: var(--yellow-d); }
...
tr:hover td { background: #FFFBEF; }
.tag { display: inline-block; padding: 4px 11px; border-radius: 20px; font-size: 11px; font-weight: 700; background: var(--yellow-l); color: var(--yellow-d); letter-spacing: .02em; border: 1px solid var(--yellow-m); }
.mono { font-family: 'DM Mono', monospace; }
.bold { font-weight: 700; }
.accent { color: var(--yellow-d); font-weight: 700; }
.muted { color: var(--grey-m); font-size: 12.5px; }

/* Filter bar */
.filter-bar { display: flex; gap: .75rem; flex-wrap: wrap; margin-bottom: 1.5rem; align-items: center; }
.filter-bar select { width: auto; font-size: 13px; padding: 9px 13px; border-radius: 9px; font-weight: 600; }
.filter-label { font-size: 11px; font-weight: 700; color: var(--grey-m); text-transform: uppercase; letter-spacing: .05em; }
...
.edit-btn { background: var(--yellow-l); color: var(--yellow-d); border: 1px solid var(--yellow-m); }
.edit-btn:hover { background: var(--yellow); color: var(--dark); }
.delete-btn { background: var(--red-l); color: var(--red); margin-left: 5px; border: 1px solid #FFD0CE; }
.delete-btn:hover { background: var(--red); color: #fff; }
.action-cell { display: flex; gap: 4px; align-items: center; }

/* Admin banner */
.admin-banner { background: linear-gradient(135deg, var(--dark) 0%, #2a2a10 100%); border: 1.5px solid rgba(245,184,0,.3); border-radius: var(--radius-sm); padding: 1rem 1.25rem; margin-bottom: 1.5rem; display: flex; align-items: center; gap: .75rem; }
.admin-banner span { color: #fff; font-size: 13px; font-weight: 600; flex: 1; }
.logout-btn { font-family: inherit; font-size: 12px; font-weight: 700; padding: 7px 16px; background: rgba(245,184,0,.2); color: var(--yellow); border: 1px solid rgba(245,184,0,.3); border-radius: 8px; cursor: pointer; transition: all .15s; }
.logout-btn:hover { background: var(--yellow); color: var(--dark); }

/* Modal */
.modal-overlay { position: fixed; inset: 0; background: rgba(0,0,0,.45); z-index: 1000; display: flex; align-items: center; justify-content: center; padding: 1rem; backdrop-filter: blur(4px); }
.modal-overlay.hidden { display: none; }
.modal { background: var(--white); border-radius: 20px; padding: 2rem; width: 100%; max-width: 540px; max-height: 90vh; overflow-y: auto; box-shadow: 0 20px 60px rgba(0,0,0,.2); }
.modal h2 { font-size: 19px; font-weight: 700; margin-bottom: 1.5rem; letter-spacing: -.3px; }
.modal-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 1rem; }
.modal-full { grid-column: 1/-1; }
.modal-actions { display: flex; gap: .75rem; margin-top: 1.5rem; }
.modal-actions .btn-primary, .modal-actions .btn-ghost, .modal-actions .btn-dark { margin-top: 0; flex: 1; padding: 13px 0; font-size: 14px; }

/* Employee manager */
.emp-list { display: flex; flex-direction: column; gap: 6px; max-height: 320px; overflow-y: auto; margin-bottom: 1rem; }
.emp-row { display: flex; align-items: center; justify-content: space-between; padding: 10px 14px; background: var(--grey-ll); border: 1px solid var(--border); border-radius: 9px; font-size: 14px; font-weight: 600; }
.emp-row:hover { background: var(--yellow-l); border-color: var(--yellow-m); }
.remove-emp-btn { font-family: inherit; font-size: 11px; font-weight: 700; padding: 4px 10px; border-radius: 6px; cursor: pointer; background: var(--red-l); color: var(--red); border: 1px solid #FFD0CE; transition: all .15s; }
.remove-emp-btn:hover { background: var(--red); color: #fff; }
.add-emp-row { display: flex; gap: .5rem; }
.add-emp-row input { flex: 1; }
.add-emp-btn { font-family: inherit; font-size: 13px; font-weight: 700; padding: 10px 18px; background: var(--yellow); color: var(--dark); border: none; border-radius: var(--radius-sm); cursor: pointer; white-space: nowrap; transition: all .15s; flex-shrink: 0; }
.add-emp-btn:hover { background: #FFD000; }

/* Login modal */
.login-modal { background: var(--white); border-radius: 20px; padding: 2rem; width: 100%; max-width: 380px; box-shadow: 0 20px 60px rgba(0,0,0,.2); }
.login-modal h2 { font-size: 19px; font-weight: 700; margin-bottom: .4rem; letter-spacing: -.3px; }
.login-modal p { font-size: 13px; color: var(--grey-m); margin-bottom: 1.5rem; font-weight: 500; }
.login-error { color: var(--red); font-size: 13px; margin-top: .75rem; display: none; font-weight: 600; }

/* Toast */
.toast { position: fixed; bottom: 2rem; left: 50%; transform: translateX(-50%) translateY(20px); background: var(--dark); color: #fff; padding: 13px 26px; border-radius: 40px; font-size: 14px; font-weight: 600; opacity: 0; transition: all .3s cubic-bezier(.34,1.56,.64,1); pointer-events: none; z-index: 9999; white-space: nowrap; box-shadow: 0 8px 24px rgba(0,0,0,.25); }
...
<div class="app">
  <div class="banner">
    <div class="banner-left">
      ...
      <div class="banner-text">
        ...
        <p>Shared in real time across the whole team</p>
      </div>
    </div>
    <button class="admin-pill" id="admin-toggle-btn" onclick="toggleAdminLogin()">🔐 Admin</button>
  </div>

  <div class="admin-banner" id="admin-banner" style="display:none">
    <span>🛡️ Admin mode active — edit entries, delete entries, and manage the team list</span>
    ...
    <div class="card">
      <div class="grid2">
        <div>
          ...
          <select id="log-name"><option value="">— Select your name —</option></select>
        </div>
        <div>
      ...
      <button class="btn-primary" id="submit-btn" onclick="submitEntry()">✔  Submit Entry</button>
  ...
  <div id="tab-dashboard" class="tab">
    ...
    <div class="stat-grid">
      <div class="stat-card"><div class="stat-val" id="d-total">0.0</div><div class="stat-lbl">Total Hours</div></div>
      <div class="stat-card"><div class="stat-val" id="d-count">0</div><div class="stat-lbl">Entries Logged</div></div>
      <div class="stat-card"><div class="stat-val" id="d-avg">0.0</div><div class="stat-lbl">Avg Hrs / Entry</div></div>
    </div>
...
<div class="modal-overlay hidden" id="login-modal-overlay">
  <div class="login-modal">
    <h2>🔐 Admin Login</h2>
    <p>Enter the admin password to enable edit, delete and team management access.</p>
    ...
    <div style="display:flex;gap:.75rem;margin-top:1.25rem">
      <button class="btn-ghost" style="margin-top:0;flex:1;padding:13px 0;font-size:14px" onclick="closeLoginModal()">Cancel</button>
      <button class="btn-primary" style="margin-top:0;flex:1;padding:13px 0;font-size:14px" onclick="attemptAdminLogin()">Login</button>
    </div>
  </div>
</div>
...
<div class="modal-overlay hidden" id="edit-modal-overlay">
  <div class="modal">
    ...
    <div class="modal-actions">
      ...
      <button class="btn-primary" id="save-edit-btn" onclick="saveEdit()">Save Changes</button>
    </div>
  </div>
</div>
...
<div class="modal-overlay hidden" id="delete-modal-overlay">
  <div class="modal" style="max-width:420px">
    ...
    <p style="color:var(--grey-m);font-size:14px;margin-bottom:1.5rem;font-weight:500">This will permanently delete the entry. This cannot be undone.</p>
    <div id="delete-preview" style="background:var(--grey-l);border-radius:10px;padding:1rem;font-size:13.5px;margin-bottom:1.5rem;line-height:1.9;border:1px solid var(--border)"></div>
    <div class="modal-actions">
      <button class="btn-ghost" onclick="closeDeleteModal()">Cancel</button>
      <button class="btn-dark" style="background:var(--red)" id="confirm-delete-btn" onclick="confirmDelete()">Yes, Delete</button>
    </div>
  </div>
</div>

<!-- Manage Employees Modal -->
<div class="modal-overlay hidden" id="emp-modal-overlay">
  <div class="modal" style="max-width:460px">
    <h2>👥 Manage Team</h2>
    <p style="font-size:13px;color:var(--grey-m);margin-bottom:1.25rem;font-weight:500">Add or remove team members from the dropdown list.</p>
    <div class="emp-list" id="emp-list"></div>
    <div class="add-emp-row">
      <input type="text" id="new-emp-input" placeholder="Enter new name…" onkeydown="if(event.key==='Enter')addEmployee()" />
      <button class="add-emp-btn" onclick="addEmployee()">+ Add</button>
    </div>
    <div style="margin-top:1.25rem">
      <button class="btn-ghost" onclick="closeEmpModal()">Done</button>
    </div>
  </div>
</div>
...
const DEFAULT_TEAM = ["Mitchell","Brett","Darren","Orlando","Phong","Rory","Klose","Adam","Jamie","Ben","Lachlan","Indiyah","Tahlia","Damien"];
const TEAM_KEY = "rd_team_v1";
...
// ── Team management (stored in Supabase shared storage via a special row) ──
async function loadTeam() {
  try {
    const { data } = await db.from("rd_entries").select("description").eq("name","__team_config__").limit(1);
...
async function saveTeam() {
  // Store team config as a special hidden entry
  const { data } = await db.from("rd_entries").select("id").eq("name","__team_config__").limit(1);
  if (data && data.length > 0) {
    await db.from("rd_entries").update({ description: JSON.stringify(team) }).eq("name","__team_config__");
  } else {
    await db.from("rd_entries").insert({ name:"__team_config__", date:"2000-01-01", hours:0, description: JSON.stringify(team), month:"", fy:"" });
  }
}
function populateNameDropdowns() {
  const opts = '<option value="">— Select your name —</option>' + team.map(t => `<option>${t}</option>`).join('');
  document.getElementById('log-name').innerHTML = opts;
  document.getElementById('edit-name').innerHTML = team.map(t => `<option>${t}</option>`).join('');
}
function renderEmpList() {
  document.getElementById('emp-list').innerHTML = team.map((name, i) => `
    <div class="emp-row">
      <span>${name}</span>
      <button class="remove-emp-btn" onclick="removeEmployee(${i})">Remove</button>
    </div>`).join('');
}
...
async function addEmployee() {
  ...
  if (team.includes(name)) return showToast("Name already exists.", "error");
  team.push(name);
  await saveTeam();
  populateNameDropdowns();
  renderEmpList();
...
function populateFilters() {
  const realEntries = allEntries.filter(e => e.name !== "__team_config__");
  const months = [...new Set(realEntries.map(e => e.month))].sort((a,b) => new Date("1 "+a) - new Date("1 "+b));
  const fys = [...new Set(realEntries.map(e => e.fy))].sort();
  const people = [...new Set(realEntries.map(e => e.name))].sort();
  ...
  ["d-person","e-person"].forEach(id => {
    const sel = document.getElementById(id);
    const val = sel.value;
    sel.innerHTML = '<option value="">All People</option>' + people.map(p => '<option' + (p===val?' selected':'') + '>' + p + '</option>').join('');
  });
}
...
async function submitEntry() {
  ...
  showToast("Saved! " + name + " — " + fmtDate(date) + " — " + hours + " hrs", "success");
...
function openDeleteModal(id) {
  ...
  document.getElementById("delete-preview").innerHTML =
    "<strong>" + e.name + "</strong> &mdash; " + fmtDate(e.date) + "<br>" +
    Number(e.hours).toFixed(1) + " hrs &mdash; " + e.month + " (" + e.fy + ")<br>" +
    "<span style='color:var(--grey-m)'>" + e.description + "</span>";
...
function renderEntries() {
  ...
  const adminActions = isAdmin
    ? '<button class="btn-dark" style="background:transparent;color:var(--yellow-d);border:1.5px solid var(--yellow-m);border-radius:9px;padding:9px 18px;font-size:12px;font-weight:700;cursor:pointer;margin-left:auto;white-space:nowrap" onclick="openEmpModal()">👥 Manage Team</button>'
    : '';
  // Show manage team button in filter bar when admin
  ...
  document.querySelector("#entries-table tbody").innerHTML = rows.length
    ? rows.map(e =>
        ...
        (isAdmin
          ? "<td><div class='action-cell'><button class='action-btn edit-btn' onclick='openEditModal(" + e.id + ")'>✏️ Edit</button><button class='action-btn delete-btn' onclick='openDeleteModal(" + e.id + ")'>🗑️ Delete</button></div></td>"
...
document.getElementById("emp-modal-overlay").addEventListener("click", function(e) { if (e.target===this) closeEmpModal(); });

// Add manage team button to All Entries filter bar
document.querySelector('#tab-data .filter-bar').insertAdjacentHTML('beforeend',
  '<button id="manage-team-btn" class="action-btn edit-btn" style="display:none;margin-left:auto;padding:9px 16px;font-size:12.5px" onclick="openEmpModal()">👥 Manage Team</button>'
);
